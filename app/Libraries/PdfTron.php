...
function VerifySimple($in_docpath, $in_public_key_file_path)
{
    ...
    switch ($result) {
        case PDFDoc::e_unsigned:
            ...
            return False;
        /* e_failure == bad doc status, digest status, or permissions status (i.e. does not include trust issues, because those are flaky due to being network/config-related) */ case PDFDoc::e_failure:
        ...
        case PDFDoc::e_unsupported:
            /* If necessary, call GetUnsupportedFeatures on VerificationResult to check which unsupported features were encountered (requires verification using 'detailed' APIs) */ echo(nl2br("At least one signature contains unsupported features." . PHP_EOL));
            return False;
...
function VerifyAllAndPrint($in_docpath, $in_public_key_file_path)
{
    ...
    while ($digsig_fitr->HasNext()) {
        ...
        if (count(((array) $unsupported_feature)) > 0) {
...
function CertifyPDF($in_docpath, $in_cert_field_name, $in_private_key_file_path, $in_keyfile_password, $in_appearance_image_path, $in_outpath)
{
...
function SignPDF($in_docpath, $in_approval_field_name, $in_private_key_file_path, $in_keyfile_password, $in_appearance_img_path, $in_outpath)
{
    echo(nl2br('================================================================================' . PHP_EOL));
...
function ClearSignature($in_docpath, $in_digsig_field_name, $in_outpath)
{
    echo(nl2br('================================================================================' . PHP_EOL));
...
function PrintSignaturesInfo($in_docpath)
{
    ...
    while ($digsig_fitr->HasNext()) {
        ...
        if (!$digsigfield->HasCryptographicSignature()) {
            echo(nl2br("Either digital signature field lacks a digital signature dictionary, " . "or digital signature dictionary lacks a cryptographic Contents entry. " . "Digital signature field is not presently considered signed.\n" . "==========" . PHP_EOL));
            $digsig_fitr->Next();
            continue;
        }
...
function TimestampAndEnableLTV($in_docpath, $in_trusted_cert_path, $in_appearance_img_path, $in_outpath)
{
    $doc = new PDFDoc($in_docpath);
